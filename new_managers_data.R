...
new_managers_data <- data.frame(new_managers_data)
...
getwd()
#choose record we need to merge
...
# we can create blank columns and import into data frame
...
# Default R structure is yyyy-mm-dd
...
#assign restructured date into the date field of the include list
str(include_list$Date)
include_list$Date <- restructured_date
str(include_list$Date)

#convert the date filed of the managers to be in the same format as include_list
# Managers date filed is in structured yyyy-dd-mm
str(managers$Date)
managers_converted_date <-as.Date(managers$Date, "%Y-%d-%m")
str(managers_converted_date)

#Assign new date to managers date filed
managers$Date <- managers_converted_date
str(managers$Date)

#Merge both dataset together - rbind
nrow(managers)
nrow(include_list)
